#--run likelihood profile on pLnR[2] (ln-scale mean recruitment 1975+) parameter
#--Notes:
#--1. modify MPI file:
...
##--NOTE: mpi.dat values here are on the parameter scale
base<-6.01616932310;#--base case (limits: 0   10)
cases1<-seq(from= 6.10,to= 8.00,by= 0.05);
cases2<-seq(from= 6.00,to= 4.00,by=-0.05);
...
for (ic in 1:length(casesUp)){
  ...
  if (!dir.exists(fldr)) {
    ...
    if (ic==1){
      parp = par;
      parp[id_par] = case; #--NOTE: no transform to parameter scale!
      parp = paste0(parp," \n",collapse="") |>
