#--run likelihood profile on pDM[1] (base M) parameter
#--Notes:
#--1. modify MPI file:
...
#       to parameter value in all instances (look at SEVERAL(!!) "NOTE"s below)

os<-"osx";#--or "win" or "osx"
top<-getwd();
...
#--identify cases and run models for pDM1[1]
##--NOTE: mpi.dat values here are ARITHMETIC, so no transform to parameter scale
base = 1.10373680141;#--base case (limits: 0.135335283   2.718281828)
cases1<-seq(from= 1.11,to= 1.99,by= 0.01);
cases2<-seq(from= 1.10,to= 0.21,by=-0.01);
...
for (ic in 1:length(casesUp)){
  ...
  if (!dir.exists(fldr)) {
    ...
    if (ic==1){
      parp = par;
      parp[id_par] = case; #--NOTE: must transform `case` to parameter scale! (scale is ARITHMETIC for pDM1)
      parp = paste0(parp," \n",collapse="") |>
