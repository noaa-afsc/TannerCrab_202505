...
```{r}
#| results: asis
  title = paste0("## Likelihood profile objective function components results for profiling on ",param," (",param_str,") {#sec-LP-OFCs-",param,"}");
  cat(title,"\n\n")
```
...
#| fig-asp: 1
  ...
  p = ggplot(dfrTs,aes(x=case,y=diff)) +
        ...
        geom_line() +
        labs(subtitle="total objective function",y="difference from base (>0: worse fit)",x=param) +
