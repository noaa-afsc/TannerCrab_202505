...
fnAllOut = file.path("run","Gmacsall.out");
iln = 1;
lstAll<-wtsGMACS::readGmacsAllout(fnAllOut)
wtsUtilities::saveObj(lstAll,"rda_Gmacsall_out.RData");
fnRep1 = file.path("run","gmacs.rep1");
iln=1;
rep =  wtsGMACS::readGmacsRep1(fnRep1,verbose=FALSE);
wtsUtilities::saveObj(rep,"rda_Gmacs_rep1.RData");
...
ggplot(dfrCatFit |> dplyr::filter(fleet %in% crbFshs,type=="retained"),
  ...
  facet_grid(sex~fleet,scales="free_y") +
  wtsPlots::getStdTheme();
ggplot(dfrCatFit |> dplyr::filter(fleet %in% crbFshs),
       aes(x=year,colour=type,shape=sex,linetype=sex)) +
  geom_point(aes(y=obs)) +
  # geom_ribbon(aes(ymin=lba,ymax=uba),fill="blue",alpha=0.3) +
  # geom_ribbon(aes(ymin=lbo,ymax=ubo),fill="green",alpha=0.3) +
  geom_line(aes(y=prd)) +
  ...
  wtsPlots::getStdTheme();
ggplot(dfrCatFit |> dplyr::filter(fleet %in% "TCF",type=="total"),
  ...
  facet_grid(sex~fleet,scales="free_y") +
  wtsPlots::getStdTheme();
ggplot(dfrCatFit |> dplyr::filter(fleet %in% "SCF",type=="total"),
       aes(x=year,colour=type,shape=sex)) +
  geom_point(aes(y=obs)) +
  # geom_ribbon(aes(ymin=lba,ymax=uba),fill="blue",alpha=0.3) +
  # geom_ribbon(aes(ymin=lbo,ymax=ubo),fill="green",alpha=0.3) +
  ...
  facet_grid(sex~fleet,scales="free_y") +
  wtsPlots::getStdTheme();
ggplot(dfrCatFit |> dplyr::filter(fleet %in% "RKF",type=="total"),
       aes(x=year,colour=type,shape=sex)) +
  geom_point(aes(y=obs)) +
  # geom_ribbon(aes(ymin=lba,ymax=uba),fill="blue",alpha=0.3) +
  # geom_ribbon(aes(ymin=lbo,ymax=ubo),fill="green",alpha=0.3) +
