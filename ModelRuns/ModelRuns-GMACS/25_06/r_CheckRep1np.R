...
if (!is.null(dirPrj)){
  resTCSAM02 = list(tcsam02=
                      wtsUtilities::getObj(file.path(dirPrj,"ModelRuns/ModelRuns-TCSAM02/2025_22_03d5p/run_results/Results.RData")));
} else {
  fn = normalizePath(file.path(dirThs,"../../ModelRuns-TCSAM02/2025_22_03d5p/run_results/Results.RData"));
...
tmp1 = getSelFcns(tmp |> dplyr::filter(stringr::str_starts(case,"gmacs")))
ggplot(tmp1,aes(x=z,y=val,colour=ylabs)) +
      geom_line() +
      geom_vline(xintercept=125,linetype=3) +
      scale_y_continuous(limits=c(0,NA)) +
      labs(x="size (mm CW)",y="Selectivity",subtitle="TCF males") +
      wtsPlots::getStdTheme();
...
compareSelFcns(tmp,"Selectivity",sub="NMFS females");
...
dfrZCsg = rep$Size_fit_summary |>
           dplyr::mutate(
             dplyr::across(c(year,size,inpSS,inpN,aggObs,aggPrd,aggRes),
                           as.numeric),
             case="gmacs"
            ) |> dplyr::rename(y=year,x=sex,m=maturity,s=shell,z=size,catch.type=comp_type);
