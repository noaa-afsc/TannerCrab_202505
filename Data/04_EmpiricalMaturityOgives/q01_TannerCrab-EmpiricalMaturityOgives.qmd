...
Determining the fraction of male crab which have undergone the terminal molt-to-maturity in a year currently relies on a statistical analysis of chela height/carapace width data collected during the NMFS EBS bottom trawl survey that year [@RicharAndFoy2022]. Chela heights prior to 2006, when collected, were measured to the nearest 1-mm while those since have been measured to the nearest 0.1-mm. The measurements accurate to only 1-mm introduce substantial quantization effects into the data and are not included in the results J. Richar (AFSC Kodiak) provides to the author. In addition, the results are provided at 10-mm size intervals while the assessment is conducted using 5-mm size bins. Consequently, I fit Richar's estimates, weighted by the number of crab measured in each 10-mm size bin, using a smooth function of size and annual "factor smooth" ("fs") curves using the R package `mgcv` ([@mgcv]) to interpolate his results to 5-mm size bins and estimate a mean curve applicable to years in which appropriate chela height data were not collected ([@fig_EMOs-MaleMaturityOgives]). The model fit the Richar's results quite well, with an $R^2$ of 97.1%.
