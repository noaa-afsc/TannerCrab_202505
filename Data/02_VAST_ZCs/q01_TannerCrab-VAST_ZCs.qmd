...
Comparisons of the time series of summed size compositions by year and population category better reveal the differences in overall scale between the design-based and `VAST` size compositions ([@fig-VAST-ZCs-TotAbund]). The design-based values can be substantially different from the `VAST`-based values (>20% difference; [@tbl-VAST-ZCs-AbdPctDif]) and are generally larger for all three population categories, although counter-examples (1979, for example) are evident as well.
